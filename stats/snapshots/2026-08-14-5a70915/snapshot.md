...
| | |
|---|---|
| snapshot taken | 2026-08-14 23:00 EDT |
| commit | `5a7091564ccb` |
| branch | `b5-season-list-parity` |
| cells file | `not supplied` |
| model file | `/home/bbowman.guest/.claude/jobs/0590839a/tmp/snap_model.csv` |
...
- The seven clean_sheet_calibration movements are a HARNESS FIX, not the model. TestDiagCleanSheetPoisson gained the 'Fixtures != 1' guard its four siblings already carried, so 85 double-gameweek rows that compared P(no goals in BOTH matches) against 'a clean sheet in EITHER' are no longer counted. n falls 2955 to 2870 and the error rises 0.0592 to 0.0698.
- This branch changes config override semantics (analysis.CampaignMap replaces rather than merges) and moves NO model figure. That rests on MECHANISM, not on the empirical null: TestDiagCleanSheetPoisson builds no Engine, every other model diagnostic constructs an EMPTY analysis.Congestion so the campaign maps never reach the replay path, and all eight congestion penalties ship at 1.00.
...
Compared against `2026-08-14-84a0945`.

### Figures that moved

| figure | previous | now | change |
|---|---:|---:|---:|
| `model.clean_sheet_calibration.clean_sheet_rate_all_team_matches_pooled.actual` | 0.2437 | 0.2383 | -0.0054 |
| `model.clean_sheet_calibration.clean_sheet_rate_all_team_matches_pooled.error` | 0.0592 | 0.0698 | +0.0106 |
| `model.clean_sheet_calibration.clean_sheet_rate_all_team_matches_pooled.points_per_match_for_a_defender` | 0.2368 | 0.2791 | +0.0423 |
| `model.clean_sheet_calibration.clean_sheet_rate_all_team_matches_pooled.predicted` | 0.3029 | 0.3081 | +0.0052 |
| `model.clean_sheet_calibration.expected_against_actual_goals_conceded.actual` | 1.5174 | 1.4864 | -0.031 |
| `model.clean_sheet_calibration.expected_against_actual_goals_conceded.error` | -0.0212 | -0.0266 | -0.0054 |
| `model.clean_sheet_calibration.expected_against_actual_goals_conceded.predicted` | 1.4963 | 1.4599 | -0.0364 |
| `stamp.commit` | 84a0945aebb6 | 5a7091564ccb | — |
