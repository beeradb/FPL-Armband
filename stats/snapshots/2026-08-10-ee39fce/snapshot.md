...
|---|---|---|---:|---:|---:|---:|
| ORACLEMINUTES#1 | **POLICY** | ORACLE[info:minutes] perfect minutes | 107–140 pts | 50–70 pts | 2.06 | 0.149 |
| ORACLEMINUTES#1 | POLICY | *pooled over the sweep's arms* | 50–70 pts, 15 df | — | — | 0.149 (strictest arm) |
| ORACLEMINUTES#1 | **HOLD** | ORACLE[info:minutes] perfect minutes | 135–177 pts | 70–98 pts | 1.68 | 0.214 |
| ORACLEMINUTES#1 | HOLD | *pooled over the sweep's arms* | 70–98 pts, 15 df | — | — | 0.214 (strictest arm) |
| ORACLEMINUTES#1 | **armband pinned** | ORACLE[info:minutes] perfect minutes | 158–207 pts | 74–105 pts | 2.03 | 0.154 |
| ORACLEMINUTES#1 | armband pinned | *pooled over the sweep's arms* | 74–105 pts, 15 df | — | — | 0.154 (strictest arm) |
| ORACLEMINUTES#1 | **nobody doubled** | ORACLE[info:minutes] perfect minutes | 166–217 pts | 62–87 pts | 3.23 | 0.052 |
| ORACLEMINUTES#1 | nobody doubled | *pooled over the sweep's arms* | 62–87 pts, 15 df | — | — | 0.052 (strictest arm) |
...
| | |
|---|---|
| snapshot taken | 2026-08-10 21:34 EDT |
| commit | `ee39fce58c50` — **working tree was dirty**, so this commit alone does not identify the code that ran |
| branch | `worktree-oracle-harness` |
| cells file | `/tmp/verify/minutes.csv, /tmp/verify/prices.csv` |
| model file | `/tmp/model.csv` |
| inference for ORACLEMINUTES#1 | `stats/out/minutes` |
...
|---|---:|---:|---:|---|
| HOLD (hold the opening fifteen, never transfer) | 1 | 24 | 24 | moved in 24 cells (worst: 2024-25 entered at GW6, ORACLE[info:minutes] perfect minutes: 2052 against 1762) |
| nobody doubled (HOLD with no captain at all) | 1 | 24 | 24 | moved in 24 cells (worst: 2023-24 entered at GW6, ORACLE[info:minutes] perfect minutes: 1719 against 1417) |
...
- Both supplied cell grids are ORACLE sweeps (info:minutes, info:prices), so every non-baseline arm in the harness half is a hindsight upper bound and none of it is comparable with an ordinary sweep's totals.
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| ORACLEMINUTES#1 | **POLICY** | 1.265 | 0.000 | 3.016 | 1.765 | 51% | 49% | 0.149 |
| ORACLEMINUTES#1 | **HOLD** | 1.426 | 1.698 | 4.230 | 2.239 | 41% | 59% | 0.214 |
| ORACLEMINUTES#1 | **armband pinned** | 1.861 | 2.384 | 4.502 | 2.615 | 51% | 49% | 0.154 |
| ORACLEMINUTES#1 | **nobody doubled** | 2.284 | 2.078 | 3.746 | 2.749 | 69% | 31% | 0.052 |
...
Compared against `2026-08-10-ae14529`.
...
| `stamp.commit` | ae145291e803 | ee39fce58c50 | — |
...
- `harness.oracleminutes_1.mde_season.policy.oracle_info_minutes_perfect_minutes.clustered` = 140
- `harness.oracleminutes_1.mde_season.policy.oracle_info_minutes_perfect_minutes.startfixed` = 70
- `harness.oracleminutes_1.mde_season.policy` = 70
- `harness.oracleminutes_1.sig_season.policy` = 50
- `harness.oracleminutes_1.estimator.policy` = start fixed, no season effect
- `harness.oracleminutes_1.df.policy` = 15
- `harness.oracleminutes_1.mde_season.hold.oracle_info_minutes_perfect_minutes.clustered` = 177
- `harness.oracleminutes_1.mde_season.hold.oracle_info_minutes_perfect_minutes.startfixed` = 98
- `harness.oracleminutes_1.mde_season.hold` = 98
- `harness.oracleminutes_1.sig_season.hold` = 70
- `harness.oracleminutes_1.estimator.hold` = start fixed, no season effect
- `harness.oracleminutes_1.df.hold` = 15
- `harness.oracleminutes_1.mde_season.hold_fixedcap.oracle_info_minutes_perfect_minutes.clustered` = 207
- `harness.oracleminutes_1.mde_season.hold_fixedcap.oracle_info_minutes_perfect_minutes.startfixed` = 105
- `harness.oracleminutes_1.mde_season.hold_fixedcap` = 105
- `harness.oracleminutes_1.sig_season.hold_fixedcap` = 74
- `harness.oracleminutes_1.estimator.hold_fixedcap` = start fixed, no season effect
- `harness.oracleminutes_1.df.hold_fixedcap` = 15
- `harness.oracleminutes_1.mde_season.hold_nocap.oracle_info_minutes_perfect_minutes.clustered` = 217
- `harness.oracleminutes_1.mde_season.hold_nocap.oracle_info_minutes_perfect_minutes.startfixed` = 87
- `harness.oracleminutes_1.mde_season.hold_nocap` = 87
- `harness.oracleminutes_1.sig_season.hold_nocap` = 62
- `harness.oracleminutes_1.estimator.hold_nocap` = start fixed, no season effect
- `harness.oracleminutes_1.df.hold_nocap` = 15
- `harness.oracleprices_1.mde_season.policy.oracle_info_prices_perfect_timing.clustered` = 66
- `harness.oracleprices_1.mde_season.policy.oracle_info_prices_perfect_timing.startfixed` = 31
- `harness.oracleprices_1.mde_season.policy` = 31
- `harness.oracleprices_1.sig_season.policy` = 22
- `harness.oracleprices_1.estimator.policy` = start fixed, no season effect
- `harness.oracleprices_1.df.policy` = 15
- `harness.oracleprices_1.invariant.hold` = true
- `harness.oracleprices_1.invariant.hold_fixedcap` = true
- `harness.oracleprices_1.invariant.hold_nocap` = true
- `harness.mde_season.finest` = 66
- `harness.mde_season.coarsest` = 217
- `sweep.ORACLEMINUTES#1.digest` = 78a58b3f4253
- `sweep.ORACLEMINUTES#1.commit` = e51a56a66d10
- `sweep.ORACLEMINUTES#1.seasons` = 2022-23 2023-24 2024-25 2025-26
- `sweep.ORACLEMINUTES#1.starts` = 1, 6, 11, 16, 21, 26
- `sweep.ORACLEMINUTES#1.arms` = 2
- `sweep.ORACLEMINUTES#1.arms_killed` = 0
- `sweep.ORACLEMINUTES#1.arms_partial` = 0
- `sweep.ORACLEMINUTES#1.invariance_differs.hold_hold_the_opening_fifteen_never_transfer` = 24
- `sweep.ORACLEMINUTES#1.invariance_differs.nobody_doubled_hold_with_no_captain_at_all` = 24
- `sweep.ORACLEPRICES#1.digest` = 78a58b3f4253
- `sweep.ORACLEPRICES#1.commit` = e51a56a66d10
- `sweep.ORACLEPRICES#1.seasons` = 2022-23 2023-24 2024-25 2025-26
- `sweep.ORACLEPRICES#1.starts` = 1, 6, 11, 16, 21, 26
- `sweep.ORACLEPRICES#1.arms` = 2
- `sweep.ORACLEPRICES#1.arms_killed` = 0
- `sweep.ORACLEPRICES#1.arms_partial` = 0
- `sweep.ORACLEPRICES#1.invariance_differs.hold_hold_the_opening_fifteen_never_transfer` = 0
- `sweep.ORACLEPRICES#1.invariance_differs.nobody_doubled_hold_with_no_captain_at_all` = 0
- `harness.oracleminutes_1.sd_season.policy` = 1.265
- `harness.oracleminutes_1.sd_resid.policy` = 3.016
- `harness.oracleminutes_1.share_season.policy` = 51
- `harness.oracleminutes_1.sd_season.hold` = 1.426
- `harness.oracleminutes_1.sd_resid.hold` = 4.230
- `harness.oracleminutes_1.share_season.hold` = 41
- `harness.oracleminutes_1.sd_season.hold_fixedcap` = 1.861
- `harness.oracleminutes_1.sd_resid.hold_fixedcap` = 4.502
- `harness.oracleminutes_1.share_season.hold_fixedcap` = 51
- `harness.oracleminutes_1.sd_season.hold_nocap` = 2.284
- `harness.oracleminutes_1.sd_resid.hold_nocap` = 3.746
- `harness.oracleminutes_1.share_season.hold_nocap` = 69
- `harness.oracleprices_1.sd_season.policy` = 0.620
- `harness.oracleprices_1.sd_resid.policy` = 1.350
- `harness.oracleprices_1.share_season.policy` = 56
