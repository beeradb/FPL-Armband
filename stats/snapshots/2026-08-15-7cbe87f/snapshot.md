...
**Harness accuracy** asks whether the replay can see anything at all. Its grid is whatever `FPL_SWEEP_SEASONS` selected for the sweep this snapshot reads — six season pairs by default — and the per-arm thresholds below are the only authority on what it could resolve.
...
| | |
|---|---|
| snapshot taken | 2026-08-15 12:41 EDT |
| commit | `7cbe87f3edc5` |
| branch | `xpoints-position-scale` |
...
- the model CSV /home/bbowman.guest/.claude/jobs/58cd969d/tmp/model.csv holds 10 runs (newest 1786811958-1376351 wins per figure). It appends and its path is outside the repo, so confirm every run in it came from this commit — a stale run from another branch is how 2026-08-15-9e743cf shipped pre-fix clean-sheet figures under a commit that had the fix
...
Compared against `2026-08-15-5a935cf`.

### Figures that moved

| figure | previous | now | change |
|---|---:|---:|---:|
| `stamp.commit` | 5a935cf7e8c7 | 7cbe87f3edc5 | — |
