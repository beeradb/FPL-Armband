...
Each row is a **range**, because two estimators are defensible and the test that would choose between them cannot. *Clustered* treats the seasons as the independent units, which is right whenever the effect genuinely differs between them, and it costs degrees of freedom — one fewer than the number of seasons, so the multiple of the standard error needed for p = 0.05 is well above the familiar 2. The per-arm rows below carry the figure actually used. *Start-fixed* treats the entry gameweeks as the fixed device they are — the same ones are replayed in every season on purpose, so an offset between them cancels from a paired comparison — which buys five times the degrees of freedom and is valid only where the season component really is zero.
...
| MINHL#1 | **POLICY** | half-life 2 | 75–102 pts | 35–49 pts | 2.97 | 0.031 |
| MINHL#1 | **POLICY** | half-life 8 | 47–64 pts | 35–50 pts | 1.17 | 0.352 |
| MINHL#1 | **POLICY** | half-life 20 | 67–91 pts | 33–46 pts | 2.71 | 0.043 |
| MINHL#1 | POLICY | *pooled over the sweep's arms* | 76–103 pts, 5 df | — | — | 0.000 (strictest arm) |
...
**The season F test cannot be used to pick an end of the range.** It tests whether the effect differs between seasons at all, and a small p licenses the clustered figure. A large p does *not* license the other end, because on this grid that test has only **30% power** against a season component large enough to double the clustered variance — it fails to reject four times in five when the thing it looks for is there. So both are reported, with the evidence beside them, and a reader who needs one number should take the conservative end.
...
| | |
|---|---|
| snapshot taken | 2026-08-16 16:06 EDT |
| commit | `f5fba6830744` |
| branch | `integrate-the-engine-rules-pin` |
| cells file | `/home/bbowman.guest/.claude/jobs/e1fd170e/tmp/cells5.csv` |
| model file | `/home/bbowman.guest/.claude/jobs/e1fd170e/tmp/model5.csv` |
| inference for MINHL#1 | `stats/out/cells5` |
...
- the model CSV /home/bbowman.guest/.claude/jobs/e1fd170e/tmp/model5.csv holds 8 runs (newest 1786909972-2168820 wins per figure). It appends and its path is outside the repo, so confirm every run in it came from this commit — a stale run from another branch is how 2026-08-15-9e743cf shipped pre-fix clean-sheet figures under a commit that had the fix

### Sweep `MINHL#1`

| | |
|---|---|
| ran at commit | `ea8339cac565`, tree dirty |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| MINHL#1 | **POLICY** | 1.595 | 0.967 | 2.512 | 1.896 | 71% | 29% | 0.000 |
...
A season component estimated at zero is not proof of zero — a handful of seasons gives it a handful of degrees of freedom, and a component measured at zero and one as large as the whole spread are not distinguishable at that sample size. Read a zero as "the best available reading", not as a fact.
...
| points — model — Haulers | 11785 | 4.806 | 5.624 | -4.735 | 3.036 |
...
| predicted 2.0 to 3.0 | 18585 | 2.488 | 2.581 | 1.037 |
| predicted 3.0 to 4.0 | 11110 | 3.417 | 3.502 | 1.025 |
...
| CANDIDATE: appearance constants refit on the windowed population — tail and ordering | 0 | — | — | — | — | -0.161 | -0.003 |
...
| CANDIDATE: appearance constants refit against ExpectedMinutes — tail and ordering | 0 | — | — | — | — | -0.083 | 0 |
...
| CANDIDATE: rate recency, half-life 8 — tail and ordering | 0 | — | — | — | — | 0.041 | 0.001 |
...
Compared against `2026-08-16-258ad5c`.

### Figures that moved

| figure | previous | now | change |
|---|---:|---:|---:|
| `harness.mde.present` | false | true | — |
| `stamp.commit` | 258ad5ca957e | f5fba6830744 | — |
...
- `harness.minhl_1.mde_season.policy.flat_no_recency.clustered` = 139
- `harness.minhl_1.mde_season.policy.flat_no_recency.startfixed` = 39
- `harness.minhl_1.mde_season.policy.half_life_2.clustered` = 102
- `harness.minhl_1.mde_season.policy.half_life_2.startfixed` = 49
- `harness.minhl_1.mde_season.policy.half_life_8.clustered` = 64
- `harness.minhl_1.mde_season.policy.half_life_8.startfixed` = 50
- `harness.minhl_1.mde_season.policy.half_life_20.clustered` = 91
- `harness.minhl_1.mde_season.policy.half_life_20.startfixed` = 46
- `harness.minhl_1.mde_season.policy` = 103
- `harness.minhl_1.sig_season.policy` = 76
- `harness.minhl_1.estimator.policy` = season-clustered (primary)
- `harness.minhl_1.df.policy` = 5
- `harness.minhl_1.mde_season.hold.flat_no_recency.clustered` = 65
- `harness.minhl_1.mde_season.hold.flat_no_recency.startfixed` = 50
- `harness.minhl_1.mde_season.hold.half_life_2.clustered` = 62
- `harness.minhl_1.mde_season.hold.half_life_2.startfixed` = 46
- `harness.minhl_1.mde_season.hold.half_life_8.clustered` = 37
- `harness.minhl_1.mde_season.hold.half_life_8.startfixed` = 43
- `harness.minhl_1.mde_season.hold.half_life_20.clustered` = 48
- `harness.minhl_1.mde_season.hold.half_life_20.startfixed` = 53
- `harness.minhl_1.mde_season.hold` = 48
- `harness.minhl_1.sig_season.hold` = 34
- `harness.minhl_1.estimator.hold` = start fixed, no season effect
- `harness.minhl_1.df.hold` = 25
- `harness.minhl_1.mde_season.hold_fixedcap.flat_no_recency.clustered` = 65
- `harness.minhl_1.mde_season.hold_fixedcap.flat_no_recency.startfixed` = 58
- `harness.minhl_1.mde_season.hold_fixedcap.half_life_2.clustered` = 59
- `harness.minhl_1.mde_season.hold_fixedcap.half_life_2.startfixed` = 45
- `harness.minhl_1.mde_season.hold_fixedcap.half_life_8.clustered` = 24
- `harness.minhl_1.mde_season.hold_fixedcap.half_life_8.startfixed` = 41
- `harness.minhl_1.mde_season.hold_fixedcap.half_life_20.clustered` = 47
- `harness.minhl_1.mde_season.hold_fixedcap.half_life_20.startfixed` = 52
- `harness.minhl_1.mde_season.hold_fixedcap` = 49
- `harness.minhl_1.sig_season.hold_fixedcap` = 35
- `harness.minhl_1.estimator.hold_fixedcap` = start fixed, no season effect
- `harness.minhl_1.df.hold_fixedcap` = 25
- `harness.minhl_1.mde_season.hold_nocap.flat_no_recency.clustered` = 67
- `harness.minhl_1.mde_season.hold_nocap.flat_no_recency.startfixed` = 52
- `harness.minhl_1.mde_season.hold_nocap.half_life_2.clustered` = 59
- `harness.minhl_1.mde_season.hold_nocap.half_life_2.startfixed` = 45
- `harness.minhl_1.mde_season.hold_nocap.half_life_8.clustered` = 26
- `harness.minhl_1.mde_season.hold_nocap.half_life_8.startfixed` = 41
- `harness.minhl_1.mde_season.hold_nocap.half_life_20.clustered` = 48
- `harness.minhl_1.mde_season.hold_nocap.half_life_20.startfixed` = 49
- `harness.minhl_1.mde_season.hold_nocap` = 47
- `harness.minhl_1.sig_season.hold_nocap` = 33
- `harness.minhl_1.estimator.hold_nocap` = start fixed, no season effect
- `harness.minhl_1.df.hold_nocap` = 25
- `harness.mde_season.finest` = 24
- `harness.mde_season.coarsest` = 139
- `sweep.MINHL#1.digest` = 67455aaa8835
- `sweep.MINHL#1.commit` = ea8339cac565
- `sweep.MINHL#1.seasons` = 2020-21 2021-22 2022-23 2023-24 2024-25 2025-26
- `sweep.MINHL#1.starts` = 1, 6, 11, 16, 21, 26
- `sweep.MINHL#1.arms` = 5
- `sweep.MINHL#1.arms_killed` = 0
- `sweep.MINHL#1.arms_partial` = 0
- `sweep.MINHL#1.invariance_differs.hold_hold_the_opening_fifteen_never_transfer` = 136
- `sweep.MINHL#1.invariance_differs.nobody_doubled_hold_with_no_captain_at_all` = 117
- `harness.minhl_1.sd_season.policy` = 1.595
- `harness.minhl_1.sd_resid.policy` = 2.512
- `harness.minhl_1.share_season.policy` = 71
- `harness.minhl_1.sd_season.hold` = 0.000
- `harness.minhl_1.sd_resid.hold` = 2.618
- `harness.minhl_1.share_season.hold` = 0
- `harness.minhl_1.sd_season.hold_fixedcap` = 0.000
- `harness.minhl_1.sd_resid.hold_fixedcap` = 2.676
- `harness.minhl_1.share_season.hold_fixedcap` = 0
- `harness.minhl_1.sd_season.hold_nocap` = 0.000
- `harness.minhl_1.sd_resid.hold_nocap` = 2.534
- `harness.minhl_1.share_season.hold_nocap` = 0
