...
# --- MOVED FROM sweep_inference.R, 2026-08-27 -------------------------------
#
# This guard lived in sweep_inference.R, and so only ONE script could reach it.
# `gate_additivity.R` forms a cross-arm contrast out of a cells file and never
# called it — the gap named in the work item this move closes. Copying the guard
# there would have been two implementations of one quantity, which is the bug
# class the rest of this file exists to prevent, so it moves here instead and
# every script that reads cells reaches the same one.
#
