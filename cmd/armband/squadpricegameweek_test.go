...
// during the live gap only Bootstrap.CurrentEvent() (FPL's own is_current
// flag) can, which is a genuinely different signal from GameweeksPlayed(),
// not a boolean substitute for it.
func TestSquadPriceGameweekPicksTheLiveGapsCurrentEvent(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"a finished gameweek wins even if a later one is flagged current", 3, &fpl.Event{ID: 4}, 3},
