...
| `minutes_weight` | `1.0` | Convexity of the rotation penalty. 1.0 is neutral; 1.6+ is ruthless about nailed-ness. ⚠️ Was `1.25` until 2026-08-25: a Holm-corrected re-sweep found 1.25 was the worst of five tested values on a response surface this harness cannot locate an optimum on, so the user moved it to neutral rather than defend an un-locatable value. See `AGENTS.md`'s Constants section. |
