...
func TestDiagUnknownPrior(t *testing.T) {
	...
	fmt.Printf("⚠️ Pre-registered from the ORDERING instrument: the fix alone was\n")
	fmt.Printf("negative on ordering in all six seasons, and fix+price positive in all\n")
	fmt.Printf("six. So expect the fix alone to be flat-to-negative here and the\n")
	fmt.Printf("contrast between the two arms to be the positive one.\n")
