...
func (e *Engine) engineAtHorizon(gw, horizon int) *Engine {
	...
	// Read together under overrideMu, the same rule minutesOverrideFor's doc
	// comment states: three unguarded field reads here could each observe a
	// different SetMinutesOverride write (set_player_status runs concurrently
	// with other tool calls in the same turn), pairing one write's minutes
	// with another's expiry or confirmed flag.
	e.overrideMu.RLock()
