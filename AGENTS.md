...
- **`MinGainHit` 3.0 stands, the hits mostly pay, and nothing ships.** On the horizon criterion, 23.5% of hit
  ...
  (thresholds 13.4-18.6, no shape). On the **holding criterion** (the user's ruling: the hit's
  −4 is in the sum, and a hit worked iff the package is non-negative by the time the
  in-players are sold or a wildcard lands, squad contribution incl. captaincy and
  chips) **78-79% of hits clear, mean +31 to +41 net of the −4**, holds ~10 gameweeks, on all
